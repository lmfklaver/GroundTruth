function [fet, clu spktimes, wav]= ConvertKlusta2Matlab_vLi(shank,basepath,...
...
tkwik = fullfile(basepath,[basename '.kwik']);
tkwx = fullfile(basepath,[basename '.kwx']);
...
if saveFiles
    ...
    for i=1:length(cluster_names)
        [~,b] =ismember(cluster_names(i),double(clusterID));
        if isempty(b)
