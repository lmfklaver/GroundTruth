function [juxtaSpikes] = GetJuxtaSpikes_orig(data, ops)
...
SNRthr = ops.SNRthr;
stdJux = std(filtJuxta);
findSpk = find(filtJuxta>(SNRthr*stdJux));
findSpkLog = filtJuxta>(SNRthr*stdJux);

juxtaSpikes.findSpk = findSpk;
juxtaSpikes.filtJuxta = filtJuxta; 

juxtaSpikes.timestamps = data.timestamps(findSpk);
...
figure, plot(filtJuxta), hold on, plot(findSpk,repmat(8000,1,length(findSpk)),'*')
